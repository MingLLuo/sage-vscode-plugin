...
class WorkspaceIndex:
    def __init__(
        ...
        self._summary_cache_module_paths: dict[str, Path] = {}
        self._summary_cache_export_index: dict[str, set[str]] = {}
        self._summary_cache_query_index: dict[str, set[str]] = {}
    ...
    def debug_status(self) -> dict[str, object]:
        return {
            "sourceRoots": [str(root) for root in self._source_roots],
            "loadedRoots": [str(root) for root in sorted(self._loaded_roots, key=str)],
            "deferredRoots": [str(root) for root in self._deferred_roots()],
            "moduleCount": len(self._modules),
            "summaryCount": len(self._module_summaries),
            "documentOverlayCount": len(self._document_records),
            "fullyIndexed": self._fully_indexed,
            "cacheSnapshotComplete": self._cache_snapshot_complete,
            "summaryCacheLoaded": self._summary_cache_loaded,
            "summaryCacheComplete": self._summary_cache_complete,
            "summaryCacheDirty": self._summary_cache_dirty,
            "summaryCacheEntryCount": len(self._summary_cache_entries),
            "resolvedSymbolCacheCount": len(self._resolved_symbol_cache),
            "resolvedMemberCacheCount": len(self._resolved_member_cache),
        }
    ...
    def _reset_runtime_state(self) -> None:
        ...
        self._summary_cache_module_paths.clear()
        self._summary_cache_export_index.clear()
        self._summary_cache_query_index.clear()
        self._summary_cache_loaded = False
        self._summary_cache_complete = False
        self._summary_cache_dirty = False
    ...
    def _load_summary_cache_matches(self, needle: str) -> bool:
        entries = self._summary_cache_entries_for_query(needle)
        matched_any = False
        for cache_key, entry in list(entries.items()):
            ...
            self._store_module_summary(
                ...
                summary,
                fingerprint=fingerprint,
                update_summary_cache=False,
            )
            matched_any = True
        return matched_any

    def _summary_cache_entries_for_query(self, needle: str) -> dict[str, dict[str, object]]:
        if self._summary_cache_loaded:
            return self._summary_cache_entries
        complete, entries = self._query_cached_summary_entries(needle)
        if entries or complete:
            self._summary_cache_entries.update(entries)
    ...
    def _query_cached_summary_entries(self, needle: str) -> tuple[bool, dict[str, dict[str, object]]]:
        try:
            return IndexDatabase(self._summary_cache_db_path()).query_entries(needle)
        except OSError:
            return False, {}
    ...
    def _query_summaries_for_query(self, needle: str) -> Optional[dict[str, ModuleSummary]]:
        if not needle or self._fully_indexed:
            return None
        cached_summaries = self._query_summaries_from_summary_cache(needle)
    ...
    def _query_summaries_from_summary_cache(self, needle: str) -> Optional[dict[str, ModuleSummary]]:
        if self._summary_cache_loaded:
            return None
        complete, entries = self._query_cached_summary_entries(needle)
        if not entries and not complete:
            return None
        ...
        for cache_key, entry in entries.items():
            path = Path(cache_key)
            loaded = self._valid_cached_summary_for_query(path, entry, needle)
            if loaded is None:
                invalid_or_stale = True
                break
            module_name, summary = loaded
    ...
    def _valid_cached_summary_for_query(
        self,
        path: Path,
        entry: dict[str, object],
        needle: str,
    ) -> Optional[tuple[str, ModuleSummary]]:
        if path in self._module_paths or path in self._summary_paths:
            return None
        root = self._source_root_for_path(path)
        if root is None or root.resolve() in self._loaded_roots:
        ...
        if file_fingerprint(path) != fingerprint:
            return None
        summary = deserialize_module_summary(entry.get("summary"), module_name, path)
        if needle not in module_name.casefold() and not _module_summary_matches(summary, needle):
            return None
    ...
    def _summary_cache_db_path(self) -> Path:
        resolved_cache_dir = _resolve_cache_dir(self._cache_dir)
        if resolved_cache_dir is None:
            raise OSError("No writable cache directory available")
        return resolved_cache_dir / f"workspace-summary-{self._cache_digest()}.sqlite"
    ...
    def _load_cached_summary_entries(self) -> tuple[bool, dict[str, dict[str, object]]]:
        try:
            complete, entries = IndexDatabase(self._summary_cache_db_path()).load_entries()
        except OSError:
            complete, entries = False, {}
        if entries or complete:
    ...
    def _write_cached_summary_entries(
        ...
        try:
            if IndexDatabase(self._summary_cache_db_path()).write_entries(complete, entries):
                return
        except OSError:
            pass
