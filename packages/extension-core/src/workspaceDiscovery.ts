...
const RUNTIME_SOURCE_ROOT_SCRIPT = [
  ...
  "    sage = None",
  "if sage is not None:",
  "    candidates = list(getattr(sage, '__path__', []))",
  "    if getattr(sage, '__file__', None):",
  "        candidates.append(str(Path(sage.__file__).resolve().parent))",
  "    for candidate in candidates:",
  "        package = Path(candidate).resolve()",
  "        if package.is_dir() and package.name == 'sage':",
  "            roots.append(str(package.parent))",
  "unique = []",
  "seen = set()",
  "for entry in roots:",
